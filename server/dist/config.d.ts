export declare const PORT: number;
export declare const HOST: string;
export declare const AUTH_TOKEN: string;
export declare const MACARON_API_BASE: string;
export declare const MACARON_API_KEY: string;
export declare const MACARON_MODEL: string;
export declare const STT_BASE_URL: string;
export declare const STT_API_KEY: string;
export declare const STT_MODEL: string;
export declare const STT_LANGUAGE: string;
export declare const HOME: string;
export declare const CLAUDE_PROJECTS: string;
export declare const WEB_ROOT: string;
export declare const WEB_DIST: string;
//# sourceMappingURL=config.d.ts.map