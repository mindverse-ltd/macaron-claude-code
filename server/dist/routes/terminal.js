...
export async function registerTerminalRoutes(app) {
    ...
    app.get('/api/terminal/:project/:tid/stream', async (req, reply) => {
    ...
    app.post('/api/terminal/:project/:tid/input', async (req, reply) => {
        const data = typeof req.body?.data === 'string' ? req.body.data : '';
        const ok = ptyInput(req.params.tid, data);
        return reply.send({ ok });
    });
    app.post('/api/terminal/:project/:tid/resize', async (req, reply) => {
        const cols = Number(req.body?.cols);
        const rows = Number(req.body?.rows);
        const ok = ptyResize(req.params.tid, cols, rows);
        return reply.send({ ok });
    });
    app.post('/api/terminal/:project/:tid/kill', async (req, reply) => {
