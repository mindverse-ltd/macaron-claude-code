...
export async function registerGitRoutes(app) {
    const cwdOf = (project) => g.resolveProjectCwd(project);
    const fail = (reply, e) => {
        const code = e instanceof g.GitError ? 400 : 500;
        return reply.status(code).send({ error: e.message });
    };
    app.get('/api/git/:project/status', async ({ params }, reply) => {
        try {
    ...
    app.get('/api/git/:project/diff', async ({ params, query }, reply) => {
    ...
    app.post('/api/git/:project/stage', async ({ params, body }, reply) => {
    ...
    app.post('/api/git/:project/unstage', async ({ params, body }, reply) => {
        const files = Array.isArray(body?.files) ? body.files.filter((f) => typeof f === 'string') : [];
        if (files.length === 0)
            return reply.status(400).send({ error: 'files required' });
        try {
    ...
    app.post('/api/git/:project/commit', async ({ params, body }, reply) => {
    ...
    app.get('/api/git/:project/branches', async ({ params }, reply) => {
        try {
    ...
    app.post('/api/git/:project/checkout', async ({ params, body }, reply) => {
        const branch = String(body?.branch || '').trim();
        if (!branch)
            return reply.status(400).send({ error: 'branch required' });
        try {
