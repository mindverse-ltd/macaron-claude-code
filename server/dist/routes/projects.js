...
function isAllowedGitUrl(url) {
    if (url.startsWith('-'))
        return false; // never let the URL be read as a flag
...
function runGitClone(gitUrl, dest) {
...
export async function registerProjectRoutes(app) {
    ...
    app.post('/api/projects', async (req, reply) => {
        ...
        if (gitUrl) {
            ...
            if (!name)
                return reply.status(400).send({ error: 'could not derive a name from gitUrl — pass name explicitly' });
        }
        ...
        try {
            ...
            for (const sibling of await fs.readdir(PROJECTS_ROOT)) {
                if (sibling === name || sibling.startsWith('.'))
                    continue;
        ...
        if (gitUrl) {
            try {
                ...
                return reply.status(422).send({ error: `git clone failed: ${e.message}` });
            }
        }
