import { isConfigFileId, listConfigFiles, readConfigFile, writeConfigFile, } from '../lib/config-files.js';
export async function registerConfigFileRoutes(app) {
    app.get('/api/config-files', async () => ({ files: await listConfigFiles() }));
    app.get('/api/config-files/:id', async (req, reply) => {
        if (!isConfigFileId(req.params.id))
            return reply.status(404).send({ error: 'unknown config file' });
        return await readConfigFile(req.params.id);
    });
    app.put('/api/config-files/:id', async (req, reply) => {
        if (!isConfigFileId(req.params.id))
            return reply.status(404).send({ error: 'unknown config file' });
