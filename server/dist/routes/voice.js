...
export async function registerVoiceRoutes(app) {
    ...
    app.post('/api/voice/transcribe', { bodyLimit: TRANSCRIBE_BODY_LIMIT }, async (req, reply) => {
        if (!STT_API_KEY)
            return reply.status(503).send({ error: 'voice input not configured' });
        const audioB64 = String(req.body?.audio || '');
        const mimeType = String(req.body?.mimeType || 'audio/webm');
        if (!audioB64)
            return reply.status(400).send({ error: 'audio required' });
        const buf = Buffer.from(audioB64, 'base64');
        if (buf.length === 0)
            return reply.status(400).send({ error: 'audio empty or not base64' });
