import { listSkills, readSkillDetail, setSkillEnabled, createSkill, } from '../lib/skills-store.js';
export async function registerSkillRoutes(app) {
    app.get('/api/skills', async () => ({ skills: await listSkills() }));
    app.get('/api/skills/:dir', async (req, reply) => {
        const detail = await readSkillDetail(req.params.dir);
        if (!detail)
            return reply.status(404).send({ error: 'skill not found' });
        return detail;
    });
    app.put('/api/skills/:dir/enabled', async (req, reply) => {
