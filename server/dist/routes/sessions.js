import { deleteSession, duplicateSession, forkSession, readSessionMessages, resolveSessionCwd, rewindSession, writeCompactedSession, } from '../lib/session-store.js';
...
export async function registerSessionRoutes(app) {
    ...
    app.post('/api/sessions/claude/:project/:sid/fork', async (req, reply) => {
        const uuid = String(req.body?.uuid || '').trim();
        if (!uuid)
            return reply.status(400).send({ error: 'uuid required' });
        try {
