import { deleteSession, duplicateSession, listSubagents, readSessionMessages, readSubagentMessages, resolveSessionCwd, rewindSession, writeCompactedSession, } from '../lib/session-store.js';
...
export async function registerSessionRoutes(app) {
    ...
    app.get('/api/sessions/claude/:project/:sid/subagents', async ({ params }) => ({ subagents: await listSubagents(params.project, params.sid) }));
    // Read one subagent's full transcript, same shape as a normal session.
    app.get('/api/sessions/claude/:project/:sid/subagents/:agentId', async ({ params }, reply) => {
        if (!/^[A-Za-z0-9_-]+$/.test(params.agentId))
            return reply.status(400).send({ error: 'invalid subagent id' });
        try {
