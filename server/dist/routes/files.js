...
async function confine(root, rel, mode = 'target') {
...
async function loadIgnore(root, rel) {
    ...
    for (let i = 0; i <= parts.length; i++) {
        ...
        try {
            ...
            ig.add(body.split(/\r?\n/).map((line) => {
                if (!dirRel || !line || line.startsWith('#'))
                    return line;
...
async function isIgnored(root, rel) {
...
export async function registerFileRoutes(app) {
    ...
    app.get('/api/files/:project/list', async (req, reply) => {
        ...
        if (!abs)
            return reply.status(403).send({ error: 'path escapes project root' });
        let dirents;
        try {
        ...
        for (const d of dirents) {
            ...
            if (ig.ignores(test))
                continue;
            const entry = { name: d.name, path: childRel, type: isDir ? 'dir' : 'file' };
            if (!isDir) {
                try {
    ...
    app.get('/api/files/:project/read', async (req, reply) => {
        const root = await resolveProjectCwd(req.params.project);
        if (!root)
            return reply.status(404).send({ error: 'unknown project' });
        const rel = req.query.path || '';
        ...
        if (await isIgnored(root, rel))
            return reply.status(403).send({ error: 'path is ignored' });
        let st;
        try {
    ...
    app.put('/api/files/:project/write', async (req, reply) => {
        const root = await resolveProjectCwd(req.params.project);
        if (!root)
            return reply.status(404).send({ error: 'unknown project' });
        ...
        try {
            ...
            return reply.status(400).send({ error: `parent directory unusable: ${e.message}` });
        }
        try {
