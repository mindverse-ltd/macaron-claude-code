...
function isEnabled(overrides, dir) {
    return overrides[dir] !== 'off';
}
function yamlScalar(value) {
    return `'${value.replace(/'/g, "''")}'`;
}
async function readEntryInfo(dir, isSymlink, overrides) {
...
export async function readSkillDetail(dir) {
    if (!SLUG_RE.test(dir))
        return null;
    const skillMd = path.join(SKILLS_DIR, dir, 'SKILL.md');
    let text;
    ...
    let source = 'dir';
    try {
        if ((await fs.lstat(path.join(SKILLS_DIR, dir))).isSymbolicLink())
            source = 'symlink';
    }
    catch {
        /* keep default */
...
export async function setSkillEnabled(dir, enabled) {
    if (!SLUG_RE.test(dir))
        return false;
    const skills = await listSkills();
    if (!skills.some((s) => s.dir === dir))
        return false;
    const settings = await readSettings();
    const overrides = readOverrides(settings);
    if (enabled)
        delete overrides[dir];
    else
        overrides[dir] = 'off';
