...
function serialize(input, existingFrontmatter = []) {
    ...
    for (const line of existingFrontmatter) {
        const key = frontmatterKey(line);
        if (key === 'description') {
            if (!wroteDescription && input.description)
                fm.push(`description: ${JSON.stringify(input.description)}`);
            wroteDescription = true;
            continue;
        }
        if (key === 'argument-hint') {
            if (!wroteArgumentHint && input.argumentHint)
                fm.push(`argument-hint: ${JSON.stringify(input.argumentHint)}`);
    ...
    if (!wroteArgumentHint && input.argumentHint)
        fm.push(`argument-hint: ${JSON.stringify(input.argumentHint)}`);
    while (fm.length && !fm[fm.length - 1].trim())
        fm.pop();
...
export async function listCommands() {
    ...
    for (const f of entries) {
        ...
        if (!isValidName(name))
            continue; // skip namespaced/subdir shapes for now
        try {
...
export async function createCommand(name, input) {
    if (!isValidName(name))
...
export async function updateCommand(name, input) {
    if (!isValidName(name))
        return null;
    const full = fileFor(name);
    let parsed;
...
//# sourceMappingURL=commands-store.js.map