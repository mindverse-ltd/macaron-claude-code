...
async function git(cwd, args, okExitCodes = [0]) {
    try {
...
export async function status(cwd) {
    ...
    let branch = onBranch;
    if (detached)
        branch = hasCommits ? (await git(cwd, ['rev-parse', '--short', 'HEAD']).catch(() => 'HEAD')).trim() : 'HEAD';
    let ahead = 0;
    let behind = 0;
    let upstream;
...
export async function diff(cwd, file, opts = {}) {
...
export async function stage(cwd, files) {
    if (files.length === 0)
        return;
    await git(await gitRoot(cwd), ['add', '--', ...files]);
}
export async function unstage(cwd, files) {
    if (files.length === 0)
        return;
    const root = await gitRoot(cwd);
    ...
    if (hasCommits)
        await git(root, ['restore', '--staged', '--', ...files]);
    else
        await git(root, ['rm', '--cached', '-r', '--', ...files]);
}
export async function commit(cwd, message, all) {
    const args = ['commit'];
    if (all)
        args.push('-a');
    args.push('-m', message);
