...
export function subscribeSystemEvents(reply) {
    subs.add(reply);
    reply.raw.on('close', () => subs.delete(reply));
}
function broadcast(engine) {
    if (subs.size === 0)
        return;
    const prev = debounce.get(engine);
    if (prev)
        clearTimeout(prev);
    debounce.set(engine, setTimeout(() => {
        debounce.delete(engine);
        const payload = { type: 'sessions-changed', engine };
...
async function watchTree(dir, engine) {
