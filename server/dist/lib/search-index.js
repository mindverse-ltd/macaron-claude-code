...
function getDb() {
  ...
  `);
    ...
    const version = d.prepare('SELECT fts_version FROM files LIMIT 1').get()?.fts_version ?? FTS_SCHEMA_VERSION;
...
function lineToText(o) {
    ...
    if (text.startsWith('<') && text.includes('tool_result'))
        return null;
...
async function syncFile(filePath, project, sessionId) {
    let st;
    try {
    ...
    const prev = d
        ...
        .get(filePath);
    if (prev && prev.mtime_ms === st.mtimeMs && prev.size === st.size && prev.fts_version === FTS_SCHEMA_VERSION)
        return false;
    let raw;
    try {
    ...
    for (const line of raw.split('\n')) {
        ...
        try {
            ...
            if (parsed)
                rows.push({ ...parsed, uuid: o.uuid || '', ts: o.timestamp || '' });
        }
        catch {
            /* skip malformed line */
        }
    }
    const insert = d.prepare('INSERT INTO messages(text, file_path, project, session_id, cwd, role, uuid, ts) VALUES (?,?,?,?,?,?,?,?)');
    d.exec('BEGIN');
    try {
        ...
        d.prepare(`INSERT INTO files(file_path, project, session_id, cwd, mtime_ms, size, msg_count, fts_version, indexed_at)
       VALUES (?,?,?,?,?,?,?,?,?)
       ON CONFLICT(file_path) DO UPDATE SET
         ...
         indexed_at=excluded.indexed_at`).run(filePath, project, sessionId, cwd, st.mtimeMs, st.size, rows.length, FTS_SCHEMA_VERSION, Date.now());
        d.exec('COMMIT');
    }
    catch (e) {
        d.exec('ROLLBACK');
    ...
    const run = (async () => {
        ...
        for (const p of projects) {
            ...
            for (const f of files) {
                ...
                scanned++;
                try {
                    if (await syncFile(filePath, p.name, f.slice(0, -6)))
                        changed++;
                }
                catch {
        ...
        try {
            const d = getDb();
            const known = d.prepare('SELECT file_path FROM files').all();
...
export async function search(query, limit = 40) {
    ...
    const rows = d
        .prepare(`SELECT project, session_id, cwd, role, uuid, ts,
       ...
       ORDER BY score
       LIMIT ?`)
        .all(SEARCH_HL_OPEN, SEARCH_HL_CLOSE, expr, capped);
    return rows.map((r) => ({
        project: r.project,
...
export function indexStats() {
    try {
        const d = getDb();
        const files = d.prepare('SELECT count(*) AS c FROM files').get().c;
        const messages = d.prepare('SELECT count(*) AS c FROM messages').get().c;
