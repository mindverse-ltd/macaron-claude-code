        ...
        for (let i = 0; i < upto; i++) {
            ...
            if (o.type === 'session_meta' && !meta) {
                meta = {
                    id: String(p.id || ''),
                    cwd: String(p.cwd || ''),
                    gitBranch: typeof p.git?.branch === 'string'
                        ? p.git.branch
                        : undefined,
...
export function encodeCodexProjectName(cwd) {
    if (!cwd)
        return '';
...
export async function findCodexRolloutFile(sid) {
    try {
        await fs.access(CODEX_SESSIONS);
    }
    catch {
...
export async function readCodexSessionMessages(sid) {
...
export async function deleteCodexSession(sid) {
    const filePath = await findCodexRolloutFile(sid);
    if (!filePath)
        throw new Error(`codex session not found: ${sid}`);
