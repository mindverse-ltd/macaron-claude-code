...
function getEntry(tsconfig) {
    ...
    const entry = { service: ts.createLanguageService(host, ts.createDocumentRegistry()), versions, roots, cwd };
...
export function getFileDiagnostics(absPath, rootDir) {
    if (!TS_JS_EXT.has(path.extname(absPath)))
        return [];
    try {
        ...
        return all.map(toDiag).filter((d) => d !== null).slice(0, MAX_REPORTED);
    }
    catch {
        return [];
    }
...
export function reportForAgent(file, diagnostics) {
