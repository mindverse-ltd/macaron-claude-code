...
export function parse(raw, name) {
    const fm = {};
    const lists = {};
    let body = raw;
    const m = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/.exec(raw);
...
export function serialize(a) {
    ...
    if (a.model)
        lines.push(`model: ${frontmatterValue(a.model)}`);
    ...
    for (const [k, v] of Object.entries(a.extra ?? {}))
        lines.push(`${k}: ${v}`);
    lines.push('---', '');
...
export async function readAgent(name) {
    try {
...
export async function writeAgent(a) {
