    ...
    async (req, reply) => {
      ...
      (async () => {
        for await (const ev of runClaude({ prompt: text, cwd, resume: sid, model: providerModel, permissionMode, images, envOverrides: providerEnv, abortController })) {
          ...
          else if (ev.kind === 'permission_request') { safeSend({ type: 'permission_request', id: ev.id, toolName: ev.toolName, input: ev.input }); pushPermissionRequest(project, sid, ev.toolName); }
