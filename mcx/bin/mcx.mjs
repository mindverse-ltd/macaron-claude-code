#!/usr/bin/env node
// mcx — Macaron Codex WebUI launcher, published as a standalone package so
// `npx mcx@…` resolves bin name = package name (npx/npm exec pick the bin by
// package name and would otherwise fall back to `mcc`). This package ships no
// runtime of its own: it depends on `mcc` (workspace:* → same-commit pkg.pr.new
// URL) and boots mcc's prebuilt server, flipping the SPA at `/` to codex.html.
...
// Boot mcc's prebuilt server bundle (mcc has no `exports` field, so subpath
// access is allowed). Same server as mcc — only MACARON_ENGINE differs.
await import('mcc/server/dist/index.js');
